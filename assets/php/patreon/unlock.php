...
Patreon handles the rest - if the user is not logged in or registered, asks the user to register or login. If the user is not your patron, asks the user to become your patron. 
If the user doesnt have enough pledge, asks the user to pledge more. 
...
echo '<a href="'.$href.'"><img src="assets/php/patreon/assets/images/unlock_with_patreon.png"></a>';
...
// and then just check if the user is logged into Patreon by using below code. Code from down below can be placed wherever in your app, it doesnt need to be in the redirect_uri at which the Patreon user ends after oAuth. You just need the $access_token for the current user and thats it.
...
if (isset($access_token)) {
    ...
    $amount = $patron_response->included[0]->attributes->currently_entitled_amount_cents;
    echo '<br><br>';
    print_r ($amount);
    if ($amount >= 100) {
        echo("you are pledged!");
    } else {
        echo "not enough";
    }
    ...
    // For example, after redirecting the user to the final redirect url after the unified flow process earlier, you can check for user's membership at this point, and show/hide content or allow/disallow 
    // features depending on the result
}
